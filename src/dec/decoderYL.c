...
int DecodeYLMsg(const u8 *src, u32 len, void* dest)
{
    if(len < 16){
    ...
    MsgYL* dst = (MsgYL*)dest; //cast
    memset(dst, 0, sizeof(MsgYL)); //set 0
    ...
    TEST(46, 0, L_UNDEFINE)
    TEST(47, 2, L_BCD)
    TEST(48, 2, L_BCD_COMPRESS_ALIGN)
    TEST(49, 3, L_FIXED)
    TEST(50, 0, L_UNDEFINE)
    TEST(51, 0, L_UNDEFINE)
    TEST(52, 8, L_FIXED)
    TEST(53, 8, L_FIXED)
    TEST(54, 2, L_BCD)
    TEST(55, 2, L_BCD)
    TEST(56, 0, L_UNDEFINE)
